...
// Correct function to check encoder counts and correct if required
void correct(void){
...
// Function to reset boths encoder counts
...
void servo_coin()
{
    // Servo arm is initially at 80 degrees
...
void lever_coin()
{
    ...
    right_motor.Stop();
    left_motor.Stop();
    // moving servo arm for coin task
    servo_coin();
...
int main(void)
{
    ...
    left_motor.Stop();
    right_motor.Stop();
    move_forward_backward(-25 , ONEINCH * 1.5);
    turn_left(25, TURN90);
    // Moving up the ramp
    adjustMotor(55, 25*ONEINCH);
    adjustMotor(35, 28*ONEINCH);
